<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
    	...
    	/*
    		EL(Expression Language) : JSP에서 사용하는 표현식을 조금 더 간결하게
    		사용하기 위한 표현식
    		${변수명}
    		${num}
    		
    		내장객체가 필요하다.
    		(1) pageScope : 저장된 데이터를 현재 페이지에서만 공유하고 사용한다.
    		자바로 따지면 private이랑 비슷하지만 너무 폐쇄적이라 잘 안씀
    		(2)	requestScope(가장많이 사용하는 객체) : 지역개념으로 페이지가 닫히면 종료
    		- 같은 request라면 두 개의 페이지가 같은 요청을 공유할 수 있다.
    		- 따라서 request는 객체를 하나 또는 두개의 페이지 내에서 공유할 수 있다.
    		(3) sessionScope : 전역의 개념
    		- 톰캣이 실행 될 때 자동으로 만들어지는 영역
    		- session영역은 하나의 웹 브라우저당 한개의 session객체가 생성된다.
    		- 즉, 같은 웹 브라우저 내에서 요청되는 페이지들은 같은 객체를 공유하게 된다.
    		(4) applicationScope : 최소한 내가 만든 프로젝트 내에서의 모든 jsp에서
    		값을 공유하는 것이 가능
